...
function StatConsultant() {
  ...
  // Fonctions de vérification des critères pour les badges
  ...
  const isTopViewedFulfilled = () => {
    const totalProfileViews = profileStats && profileStats.data ? 
      profileStats.data.reduce((sum, value) => sum + value, 0) : 0;
    return totalProfileViews >= 1;
  };

  const isExcellentCommunicatorFulfilled = () => conversationCount > 10;

  const isTopRatedFulfilled = () => {
    const jobSuccessOk = consultantData && consultantData.jobSuccess >= 80;
    const earningsOk = earnings !== null && earnings >= 100000;
    return jobSuccessOk && earningsOk;
  };

  const isTopRatedPlusFulfilled = () => {
    const jobSuccessOk = consultantData && consultantData.jobSuccess >= 100;

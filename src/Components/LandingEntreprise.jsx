...
function LandingEntreprise() {
  ...
  const [entreprise, setEntreprise] = useState(undefined); // Changed from null to undefined
  ...
  useEffect(() => {
    const loadConsultants = async () => {
      ...
      setIsLoading(true);
      try {
        if (entreprise.typeEntreprise === "SSI") {
          const premiumData = await fetchPremiumConsultants();
          setConsultants(premiumData || []);
        } else {
          const [allConsultants, premiumConsultants] = await Promise.all([
            getAllConsultants(),
          ...
          setConsultants([...premiumList, ...nonPremiumList]);
        }
      } catch (error) {
    ...
    loadConsultants();
  // Change the dependency to only use typeEntreprise
...
}`}>{consultant.workload >0 ? 'En travail' : 'Disponible' }</p>                  <div className={styles.actionButtons} onClick={(e) => e.stopPropagation()}>

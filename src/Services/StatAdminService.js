...
const INSCRIPTIONS_API_URL = 'http://localhost:8081/api/entreprises/inscriptions';
const TOP_TALENTS_API_URL = 'http://localhost:8081/api/missions/top-talents';
...
const REVENUE_DISTRIBUTION_API_URL = 'http://localhost:8081/api/payments/revenue-distribution'; // API dédiée pour la répartition
...
export const fetchGlobalApplicationFeeStats = async (period = "6months") => {
  ...
  try {
    const response = await axios.get(
      `http://localhost:8081/api/payments/global/applicationFee?period=${period}`,
      {
        headers: {
